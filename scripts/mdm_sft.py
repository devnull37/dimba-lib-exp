...
def maskgit_generate(model, prompt_ids, gen_len, mask_id,
                     steps=32, temperature=0.7, top_k=50):
    ...
    for s in range(steps):
        ...
        logits = model.predict_token_logits(ids, max(frac, T_MIN)).float() / temperature
        if top_k:
...
def eval_step(model, tokenizer, step: int, mask_id: int):
    ...
    for p in EVAL_PROMPTS:
        ...
        if eos in toks:
            toks = toks[:toks.index(eos)]  # trim at first EOS (learned length)
        print(f"[step {step}] {p!r} -> {tokenizer.decode(toks)!r}", flush=True)
    model.train()
